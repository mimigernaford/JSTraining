...
var EPH = {
	...
	uStart			:0,
	/* =================================================================================== */
	/* 				Berechnung des Julianischen Datums fuer 12 Uhr MEZ				       */
	monate  		:["Januar","Februar","M"+String.fromCharCode(228)+"rz","April","Mai","Juni","Juli","August","September","Oktober","November","Dezember","Dezember"],
	...
	/* =================================================================================== */
	/* 	      Umwandlung ekliptikaler in aequatoriale Koordinaten fuer Helios()			   */
	...
	//falls von LunaE aufgerufen, nicht ausfuehren
		if(!arguments[1])
		{
		EPH.SonneE.stb = EPH.SonneE.RA *15;//fuer SternbildPL
	...
	EPH.MondE.stb = EPH.MondE.RA *15;//fuer SternbildPL
	...
	/* =================================================================================== */
	/* 						aktuelles Sternbild fuer Sonne und Mond 						   */
	SternbildE		:function ()
	{ 
...
/* =================================================================================== */
/* 						aktuelles Sternbild fuer Planeten 						       */
...
/* 						Klasse der Planeten											   */
	...
	(this.Name != "Erde")?EPH.Objekte.push(this):null; 
...
//console.log(laenge)
	...
	planet.Deklination  =  DklString_3 + dklsString_1 + "'";
		...
		//�bergabe-> Positionierung des Planetenzeigers
	...
	Berechnung		:function()
					{
						if(/select/.test(window.name))
						{
						//"select=1;2017"
						EPH.quartal		= window.name.split("select=")[1].split(";")[0];	
						EPH.formJahr	= window.name.split("select=")[1].split(";")[1];
						document.getElementById("quartal").value 	= EPH.quartal;
						document.getElementById("jahr").value 		= EPH.formJahr;
						window.name = "";
						}
						else
						{
						EPH.quartal		= document.getElementById("quartal").value;	
						EPH.formJahr	= document.getElementById("jahr").value;	
						}
					EPH.uStart 		= 1;						
					EPH.formMonat 	= EPH.Quartale[EPH.quartal];					
					EPH.JulianischesDatum(EPH.formTag,EPH.formMonat,EPH.formJahr);
					EPH.JDReverse();
					...
					EPH.OUTPUT[0] = EPH.JULIANISCHESDATUM_E.JDJahr;
						EPH.Objekte.forEach(function(O){
							...
							EPH.OUTPUT[ONummer] = {};
							(ONummer == 9)?EPH.LunaE(true):EPH.PlanetE(ONummer);
							var p = (ONummer!=9)?null:eval("EPH." +O.Name+ "E.Phase" );
							var s = (ONummer == 9)?eval("EPH." +O.Name+ "E.Sternbild" ):null;
							O = eval("EPH." +O.Name+ "E" );							
							EPH.OUTPUT[ONummer].Objektname = O.Name;							
							EPH.OUTPUT[ONummer].Tabelle	= [];							
							EPH.OUTPUT[ONummer].Tabelle.push( {"datum":EPH.JULIANISCHESDATUM_E.kurz,"a":O.Rektaszension,"d":O.Deklination,"p":p,"s":s} );
								for(var i=0; i<=O.anzahl;i++)
								{
								...
								posDiff				= (posDiff >180)?360-posDiff:posDiff;								
								EPH.OUTPUT[ONummer].Tabelle.push( {"datum":EPH.JULIANISCHESDATUM_E.kurz,"a":a,"d":d,"p":p,"s":s,"diff":posDiff} );
								} 
							}						
						});						
					EPH.EphemTable();											
	...
	EphemTable		:function()
					...
					var css = 	[
								...
								"#Mond::left:20px;",
								"#Mond,#Merkur,#Venus,#Mars::top:50px",
								"#Jupiter,#Saturn,#Uranus::top:645px",
								"#Neptun,#Pluto::top:849px",
								"#Merkur,#Jupiter::left:235px",
								"#Venus,#Saturn,#Neptun::left:395px",
								"#Mars,#Uranus,#Pluto::left:565px",																
								"table::position:absolute;padding:0px;border:#000 solid 1px;border-collapse:collapse",
								"caption::font:bold 12pt sans-serif",
								"thead th::border-bottom:#000 solid 1px;",
								"th::font:bold 10pt/12pt sans-serif;",
								"td::font:10pt/12pt sans-serif;text-align:right;padding-left:5px;border-bottom:#999 solid 1px;",
								".voll::color:#f00;",
								".neu::color:#00f;background-color:#666;color:#fff;font-weight:bold"								
								];
					css = css.join("|");
					//console.log(css)
					EPH.CREATE.Init(css);
					...
					EPH.OUTPUT.shift();
					EPH.rEle(document.getElementById("ephem"));
					var h1 = document.createElement("h1");
					EPH.quartal = parseInt(EPH.quartal)+1;
					var h1txt = document.createTextNode("Ephemeriden f"+String.fromCharCode(252)+"r das "+EPH.quartal+". Quartal "+jahr);
					h1.appendChild(h1txt)
					document.getElementsByTagName("body")[0].appendChild(h1);
					var theader = ["Dat.","RA","Dkl"];
					EPH.OUTPUT.forEach(function(O){
						var tabelle = document.createElement("table");
						tabelle.setAttribute("cellpadding","0");
						tabelle.setAttribute("cellspacing","1");
						...
						var trh = document.createElement("tr");
							...
							if(O.Objektname == "Mond")
							{
							var phaseTH = document.createElement("th");
							var phaseTX = document.createTextNode("Phase");
							phaseTH.appendChild(phaseTX);
							trh.appendChild(phaseTH);	
							}
						...
						var tbody = document.createElement("tbody");
							...
							var tr = document.createElement("tr");
								...
								{
									if(row[tupel] != null && tupel != "diff")
									{										
									var zelle = document.createElement("td"); 
									zelle.style.color = (row["diff"] <4 && O.Objektname != "Mond" )?"#ccc":null;
									...
									var daten = (tupel == "p")
										?(function(){
											...
											tr.className = (parseFloat(row[tupel]*100) >98.75)
											?"voll"
											:(parseFloat(row[tupel]*100) <1.75)
												?"neu"
												:"";
											return (wert_1 == 0)?wert_a:wert_1+"%";											 
											}())
										:row[tupel];							
									var wert = document.createTextNode(daten);																												
...
var autoload = (function(){
	window.setTimeout((function(){(EphemOnload == 1)
		?(function(){
			EPH.Init();
			(/select/.test(window.name))?EPH.Berechnung():null;
			}())
		:null;}),1000);
}());
//############################################################
